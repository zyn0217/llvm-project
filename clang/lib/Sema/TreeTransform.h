...
TreeTransform<Derived>::TransformLambdaExpr(LambdaExpr *E) {
  ...
  // pattern that would be expanded by an outer fold expression.
  //
  // We don't have a way to propagate up the ContainsUnexpandedParameterPack
  // flag from a Stmt, so we have to visit the lambda again.
  if (!LSICopy.ContainsUnexpandedParameterPack) {
    ...
    getSema().collectUnexpandedParameterPacksFromLambda(NewCallOperator,
                                                        UnexpandedPacks);
    // FIXME: Should we call DiagnoseUnexpandedParameterPacks() instead? That
    // requires the LambdaScopeInfo to exist, while it has been removed by
    // ActOnFinishFunctionBody(), unfortunately.
